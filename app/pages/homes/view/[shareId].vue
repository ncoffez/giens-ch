...
const isExpired = computed(() => {
	...
	return new Date(share.value.expiresAt) < new Date();
});
...
<template>
	<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
		...
		<div v-else-if="error || isExpired" class="min-h-screen flex items-center justify-center px-4">
			...
			<div class="max-w-screen-xl mx-auto px-4 py-8 space-y-8">
				...
				<div class="grid lg:grid-cols-3 gap-8">
					<div class="lg:col-span-2 space-y-8">
						...
						<section v-else class="aspect-video rounded-3xl bg-gradient-to-br from-primary-100 to-primary-200 dark:from-primary-900/30 dark:to-primary-800/20 flex items-center justify-center">
							<div class="text-center space-y-4">
								<UIcon name="i-lucide-home" class="w-16 h-16 mx-auto text-primary opacity-50" />
								<p class="font-bold text-primary/70">{{ home.name }}</p>
							</div>
						</section>
						...
						<section v-if="home.mustKnows?.length" class="bg-amber-50 dark:bg-amber-900/10 rounded-2xl p-6 border border-amber-200 dark:border-amber-800">
							<div class="flex items-center gap-3 mb-4">
								...
								<h2 class="text-lg font-bold text-amber-800 dark:text-amber-200">Wichtig zu wissen</h2>
							</div>
							<ul class="space-y-2">
								<li v-for="(item, index) in home.mustKnows" :key="index" class="flex items-start gap-3">
									<UIcon name="i-lucide-check-circle" class="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
									<span class="text-amber-900 dark:text-amber-100">{{ item }}</span>
								</li>
							</ul>
						</section>
